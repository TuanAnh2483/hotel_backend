...
export const chatService = {
  customer({ message, sessionId }) {
    return apiClient.post("/api/chat/customer", { message, sessionId });
  },
  partner({ message, sessionId }) {
    return apiClient.post("/api/chat/partner", { message, sessionId });
  },

  /**
   * Streaming qua SSE (fetch + ReadableStream — axios không stream được trong browser).
   * Gọi callback: onDelta(text), onTool({name}), onDone({sessionId}).
   * Throw nếu response không OK (vd 401/5xx) để caller fallback sang endpoint non-stream.
   */
  streamCustomer(args) {
    return streamChat("/api/chat/customer/stream", args, false);
  },
  streamPartner(args) {
...
async function streamChat(path, { message, sessionId, onDelta, onTool, onDone }, withAuth) {
  ...
  for (;;) {
    ...
    while ((sep = buffer.indexOf("\n\n")) !== -1) {
      ...
      dispatchEvent(rawEvent, { onDelta, onTool, onDone });
    }
  }
}

function dispatchEvent(rawEvent, { onDelta, onTool, onDone }) {
