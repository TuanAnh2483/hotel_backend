...
public class AuthService {
    ...
    public AuthResponse login(LoginRequest log) {
        ...
        // Kiểm tra cả khóa tài khoản lẫn xác minh email trước khi cấp JWT.
        assertActive(user);
    ...
    public VerifyEmailResponse verifyEmail(@Valid VerifyEmailRequest request) {
        // Client gửi token gốc; DB chỉ lưu hash của token.
    ...
    public ResendVerificationResponse resendVerification(@Valid ResendVerificationRequest request) {
        ...
        // Trả cùng response cho email không tồn tại lẫn đã xác minh để tránh dò email.
    ...
    private RegisterResponse registerWithRole(RegisterRequest req, UserType role) {
        ...
        // Đăng ký lại tài khoản chưa xác minh chỉ làm mới link, không thay đổi chủ sở hữu.
    ...
    private User createUser(String email, String rawPassword, UserType role) {
        ...
        // Kích hoạt tài khoản theo status; xác minh email theo emailVerifiedAt riêng biệt.
    ...
    private EmailVerificationDispatch issueEmailVerification(User user) {
        // Xóa link cũ trước khi tạo mới; chỉ link mới nhất có hiệu lực.
    ...
    private String hashToken(String token) {
        try {
            // Chỉ lưu hash để token gốc không thể bị lộ từ DB.
